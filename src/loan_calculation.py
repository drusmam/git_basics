...
def annuitent(summ, percent, years):
  k = (percent * (1 + percent) ** years) / ((1 + percent) ** years - 1)
  a = round(k * summ, 2)
  return a

s = float(input('Введите сумму кредита: '))
n = int(input('На какой срок?: '))
i = float(input('Какой процент годовых?: '))
i = i / 100
a = annuitent(s,i,n)

for annual in range(1, 3):
  percent = s * i
  bodyOfLoan = a - percent
  print(f'Период: {annual} Остаток долга на начало периода: {s} Выплачено процентов: {percent} Выплачено тела кредита: {bodyOfLoan}')
  s -=  bodyOfLoan

print('=' * 20)

n = n - 2 + int(input('На какой срок хотите увеличить погашение?: '))
i += (float(input('На сколько увеличен процент годовых?: ')))/100
a = annuitent(s,i,n)
print('a=',a)
print('i=',i)
print('n=',n)
for annual in range(3, n + 3 ):
  percent = s * i
  bodyOfLoan = a - percent
  print(f'Период: {annual} Остаток долга на начало периода: {s} Выплачено процентов: {percent} Выплачено тела кредита: {bodyOfLoan}')
  s -=  bodyOfLoan
